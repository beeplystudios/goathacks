...
export default function Screen() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            const { status: status_ } = await Location.requestForegroundPermissionsAsync();
            ...
            const distance = getDistance({ lat: location.coords.latitude, lon: location.coords.longitude }, {
                lat: dest.lat,
                lon: dest.lon
            })
            ...
            if (!initialLocation) {
                setInitialLocation({ lat: location.coords.latitude, lon: location.coords.longitude })
            }
    ...
    return (
        <SafeAreaView>
            <Text>{data.name}</Text>
            {initialLocation && (
                <StopView origin={initialLocation} dest={{ lat: dest.lat, lon: dest.lon }} />
            )}
            {completedStepOne && (
                <Text className="font-bold text-red-300 text-2xl">COMPLETED STEP ONE!!</Text>
            )}
        </SafeAreaView> 
    )
} 